...
import { CalendarDays, ChevronLeft, ChevronRight, Clock, GraduationCap, ScrollText } from 'lucide-vue-next';
...
<template>
    <div class="flex h-full flex-col gap-5 p-4 md:p-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-muted-foreground">Schedules from your enrolled classes</p>
                </div>
            </div>
            <!-- Legend -->
        ...
        <!-- No classes notice -->
        <div v-if="props.classes.length === 0" class="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:bg-amber-950/20 dark:border-amber-800">
            <GraduationCap class="h-4 w-4 shrink-0 text-amber-600 dark:text-amber-400" />
            <p class="text-sm text-amber-700 dark:text-amber-300">
                You have no active classes. <a :href="classesIndex()" class="font-semibold underline underline-offset-2">Manage your classes</a> to see defense schedules here.
            </p>
        </div>
        ...
        <div class="rounded-2xl border border-border bg-card overflow-hidden">
            ...
            <div class="grid grid-cols-7 auto-rows-[minmax(90px,1fr)]">
                <div
                    ...
                    :key="idx"
                    class="border-b border-r border-border/50 p-1.5"
        ...
        >
            ...
            <p class="mt-1 text-xs text-muted-foreground">
                No outline or final defenses are scheduled for {{ MONTH_NAMES[props.month - 1] }} {{ props.year }}.
            </p>
        </div>
