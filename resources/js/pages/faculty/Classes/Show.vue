...
const generateJoinCode = () => {
    generateJoinCodeForm.post(
        SchoolClassController.generateJoinCode.url({
...
const revokeJoinCode = () => {
    if (
        confirm(
            'Are you sure you want to revoke this join link? Students will no longer be able to use it to join.',
        )
    ) {
        revokeJoinCodeForm.delete(
            SchoolClassController.revokeJoinCode.url({
                class: props.schoolClass.id,
            }),
            {
                preserveScroll: true,
...
const removeStudent = (studentId: number) => {
    if (
        ...
        removeStudentForm.delete(
            SchoolClassController.removeStudent.url({
                class: props.schoolClass.id,
                student: studentId,
            }),
            {
