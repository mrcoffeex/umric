...
import { Users, Copy, RefreshCw, Unlink, UserMinus, ChevronLeft, ScrollText, Link2, CheckCircle2, GraduationCap } from 'lucide-vue-next';
...
import { index as classResearchIndex } from '@/routes/faculty/classes/research';
...
const joinUrl = computed(() => {
    return props.schoolClass.join_code
...
const generateJoinCode = () => {
    generateJoinCodeForm.post(SchoolClassController.generateJoinCode.url({ class: props.schoolClass.id }), {
        preserveScroll: true,
    });
};

const revokeJoinCode = () => {
    if (confirm('Are you sure you want to revoke this join link? Students will no longer be able to use it to join.')) {
        revokeJoinCodeForm.delete(SchoolClassController.revokeJoinCode.url({ class: props.schoolClass.id }), {
            preserveScroll: true,
        });
    }
};
...
const removeStudent = (studentId: number) => {
    if (confirm('Are you sure you want to remove this student from the class?')) {
        removeStudentForm.delete(SchoolClassController.removeStudent.url({ class: props.schoolClass.id, student: studentId }), {
            preserveScroll: true,
        });
    }
};
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <div class="flex items-center gap-2">
            <Link :href="classesIndex()" class="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors">
                ...
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <h1 class="text-xl font-bold text-foreground truncate">{{ props.schoolClass.name }}</h1>
                                <p v-if="props.schoolClass.class_code" class="text-xs font-mono text-muted-foreground">{{ props.schoolClass.class_code }}</p>
                            </div>
                        </div>
                        <div class="mt-3 flex flex-wrap gap-1.5 uppercase">
                            <span class="rounded-full bg-muted px-2.5 py-0.5 text-[11px] font-semibold text-muted-foreground">Sec {{ props.schoolClass.section }}</span>
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-3">
            ...
            <div class="col-span-2 rounded-2xl border border-border bg-card p-4 lg:col-span-1">
                ...
                <p class="mt-1 text-2xl font-bold" :class="props.schoolClass.join_code ? 'text-green-700 dark:text-green-300' : 'text-slate-400 dark:text-slate-600'">
        ...
        <div class="grid gap-6 lg:grid-cols-5">
            ...
            <div class="lg:col-span-3 space-y-6">
                ...
                <div class="rounded-2xl border border-border bg-card overflow-hidden">
                    ...
                    <div class="p-5">
                        <div v-if="props.schoolClass.join_code" class="space-y-4">
                            <!-- Visible readonly URL -->
                            <div class="group relative rounded-xl border border-border bg-muted overflow-hidden">
                                <div class="flex items-center gap-3 p-3">
                                    <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-teal-50 dark:bg-teal-950/30">
                                        <Link2 class="h-4 w-4 text-teal-500" />
                                    </div>
                                    <p class="flex-1 min-w-0 truncate font-mono text-sm text-muted-foreground select-all cursor-text" :title="joinUrl">
                                        {{ joinUrl }}
                                    </p>
                                    <Button
                                        size="sm"
                                        :variant="copied ? 'default' : 'outline'"
                                        :class="copied ? 'bg-green-500 hover:bg-green-600 border-0 text-white' : ''"
                                        @click="copyJoinLink"
                                        type="button"
                                    >
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <Button size="sm" variant="ghost" @click="revokeJoinCode" :disabled="revokeJoinCodeForm.processing" class="text-destructive hover:text-destructive hover:bg-destructive/10">
                        ...
                        <div v-else class="flex flex-col items-center rounded-xl border border-dashed border-border p-8 text-center">
                            <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-orange-50 dark:bg-orange-950/30 mb-4">
                                <Link2 class="h-6 w-6 text-orange-400" />
                            </div>
                            <p class="text-sm font-semibold text-foreground">No join link generated yet</p>
                            <p class="mt-1 text-xs text-muted-foreground max-w-xs">Generate a link so students can enroll themselves into this class.</p>
                            <Button @click="generateJoinCode" :disabled="generateJoinCodeForm.processing" class="mt-4 bg-orange-500 text-white hover:bg-orange-600 border-0 shadow shadow-orange-500/20">
                                Generate Join Link
                            </Button>
                        </div>
                    </div>
                ...
                <div class="rounded-2xl border border-border bg-card overflow-hidden">
                    ...
                    <div class="p-5 flex flex-wrap items-center justify-between gap-3">
                        ...
                        <Button as-child size="sm" class="bg-orange-500 text-white hover:bg-orange-600 border-0 shadow shadow-orange-500/20">
                            <Link :href="classResearchIndex.url({ class: props.schoolClass.id })">
            ...
            <div class="lg:col-span-2 rounded-2xl border border-border bg-card overflow-hidden flex flex-col max-h-[700px]">
                <div class="flex items-center justify-between gap-2 border-b border-border px-5 py-4 shrink-0">
                    <div class="flex items-center gap-2">
                ...
                <div class="overflow-y-auto flex-1">
                    <div v-if="props.students.length === 0" class="flex flex-col items-center justify-center p-8 text-center h-full min-h-[200px]">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-teal-50 dark:bg-teal-950/30 mb-4">
                    ...
                    <ul v-else class="divide-y divide-border/50">
                        <li v-for="student in props.students" :key="student.id" class="flex items-center gap-3 px-5 py-3.5 transition-colors hover:bg-muted/50">
                            ...
                            <span class="hidden text-[10px] text-muted-foreground sm:block whitespace-nowrap">{{ formatDate(student.joined_at) }}</span>
                            <Button variant="ghost" size="icon" class="h-7 w-7 text-muted-foreground hover:text-red-500 shrink-0" @click="removeStudent(student.id)" title="Remove student">
