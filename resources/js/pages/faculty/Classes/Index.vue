...
import { index as classesIndex, show as classesShow } from '@/routes/faculty/classes';
...
const activeCount = computed(() => props.classes.filter((c) => c.is_active).length);
...
function submit() {
    if (editingClass.value) {
        form.patch(SchoolClassController.update.url({ class: editingClass.value.id }), {
            onSuccess: () => {
...
function deleteClass(schoolClass: SchoolClassItem) {
    ...
    useForm({}).delete(SchoolClassController.destroy.url({ class: schoolClass.id }));
}
...
function semesterLabel(semester: number | null): string {
    if (!semester) return '-';
    const labels: Record<number, string> = { 1: '1st Sem', 2: '2nd Sem', 3: 'Summer' };
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <section class="overflow-hidden rounded-2xl border border-border bg-card">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 p-5">
                <div>
                    ...
                    <p class="mt-0.5 text-sm text-muted-foreground">Create and manage your research classes.</p>
                </div>
                <Button
        ...
        <div class="grid gap-3 sm:grid-cols-3">
            <div class="flex items-center gap-3 rounded-xl border border-border bg-card p-4">
                ...
                <div>
                    <p class="text-xs font-medium text-muted-foreground">Total Classes</p>
            ...
            <div class="flex items-center gap-3 rounded-xl border border-border bg-card p-4">
                ...
                <div>
                    ...
                    <p class="text-lg font-bold text-foreground">{{ classes.reduce((s, c) => s + c.members_count, 0) }}</p>
                </div>
            </div>
        </div>
        ...
        <div
            class="grid gap-6"
            :class="showForm ? 'xl:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)]' : 'grid-cols-1'"
        >
            <!-- Left: list -->
            <div class="space-y-4">
                <div class="relative">
                ...
                <div v-if="classes.length === 0" class="flex flex-col items-center rounded-2xl border border-dashed border-border bg-card py-16">
                    <div class="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-orange-50 dark:bg-orange-950/30">
                        <GraduationCap class="h-7 w-7 text-orange-400" />
                    </div>
                    <p class="font-semibold text-foreground">No classes yet</p>
                    <p class="mt-1 text-sm text-muted-foreground">Create your first class to get started.</p>
                </div>

                <!-- Empty state: search no results -->
                <div v-else-if="filteredClasses.length === 0" class="rounded-2xl border border-dashed border-border bg-card p-8 text-center">
                    <p class="text-sm font-medium text-foreground">No classes match your search.</p>
                    <button class="mt-2 text-xs text-orange-500 hover:underline" @click="classSearch = ''">Clear search</button>
                </div>
                            ...
                            >
                                ...
                                <div class="relative shrink-0">
                                    <div
                                        ...
                                        :class="item.is_active
                                            ...
                                            : 'bg-muted'"
                                    >
                                        <GraduationCap class="h-6 w-6" :class="item.is_active ? 'text-orange-500' : 'text-muted-foreground'" />
                                    </div>
                                    <span
                                        ...
                                        :class="item.is_active ? 'bg-teal-500' : 'bg-muted-foreground'"
                                    >
                                        {{ item.members_count }}
                                    </span>
                                </div>
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span class="truncate text-base font-bold text-foreground group-hover:text-orange-600 dark:group-hover:text-orange-400">
                                            {{ item.name }}
                                        </span>
                                        <span
                                        ...
                                        <span v-if="item.subjects.length > 3" class="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">
                                            +{{ item.subjects.length - 3 }}
                                        </span>
                                        <span v-if="item.section" class="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">
                                            Sec {{ item.section }}
                                        </span>
                                        <span
                                        ...
                                        >
                                            <span class="h-1.5 w-1.5 rounded-full" :class="item.is_active ? 'bg-green-500' : 'bg-red-500'" />
                                            {{ item.is_active ? 'Active' : 'Inactive' }}
                                        </span>
                                    </div>

                                    <!-- Class code -->
                                    <p v-if="item.class_code" class="mt-0.5 text-xs font-medium text-muted-foreground/70">
                                        {{ item.class_code }}
                                    </p>

                                    <!-- Metadata chips -->
                                    <div class="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                                        <span v-if="item.school_year" class="inline-flex items-center gap-1">
                                            <Calendar class="h-3 w-3" />
                                            {{ item.school_year }}
                                        </span>
                                        <span v-if="item.semester" class="inline-flex items-center gap-1">
                                            <BookOpen class="h-3 w-3" />
                                            {{ semesterLabel(item.semester) }}
                                        </span>
                                        <span class="inline-flex items-center gap-1">
                                            <Users class="h-3 w-3" />
                                            {{ item.members_count }} student{{ item.members_count === 1 ? '' : 's' }}
                                        </span>
                                        <span v-if="item.join_code" class="inline-flex items-center gap-1 text-green-600 dark:text-green-400">
                                            <Link2 class="h-3 w-3" />
                                            Join link active
                                        </span>
                                    </div>

                                    <p v-if="item.description" class="mt-2 line-clamp-2 text-xs leading-relaxed text-muted-foreground">{{ item.description }}</p>
                                </div>

                                <!-- Hover arrow -->
                                <ExternalLink class="mt-1 h-4 w-4 shrink-0 text-muted-foreground/0 transition-all group-hover:text-muted-foreground" />
                            </Link>

                            <!-- Action bar -->
                            <div class="flex items-center justify-between border-t border-border/50 px-4 py-1.5">
                                <span class="text-[10px] font-medium tracking-wide text-muted-foreground/50 uppercase">
                                    ...
                                    <Tooltip v-if="item.join_code">
                                        <TooltipTrigger as-child>
                                            <button
                                                ...
                                                @click.stop="copyJoinLink(item.id, item.join_code!)"
                                            >
                                                <Check v-if="copiedId === item.id" class="h-3 w-3" />
            ...
            <div v-if="showForm" class="rounded-2xl border border-border bg-card p-5">
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        <h2 class="text-base font-bold text-foreground">
                            {{ editingClass ? 'Edit Class' : 'New Class' }}
                        </h2>
                        <p class="mt-0.5 text-xs text-muted-foreground">Fill in the class details below.</p>
                    </div>
                    <button type="button" class="rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground" @click="showForm = false">✕</button>
                </div>

                <form class="space-y-4" @submit.prevent="submit">
                    <div>
                        <Label>Subject <span class="text-red-500">*</span></Label>
                        <div class="mt-1.5 overflow-hidden rounded-xl border border-input">
                            ...
                            <div class="max-h-40 space-y-0.5 overflow-y-auto bg-background p-1.5">
                                <label
                                    v-for="subject in filteredSubjects"
                                    :key="subject.id"
                                    class="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-muted"
                                >
                                    <input v-model="form.subject_id" type="radio" :value="subject.id" class="accent-orange-500" />
                                    <span class="flex-1 text-sm">
                                        <span class="font-medium">{{ subject.code }}</span>
                                        <span class="text-muted-foreground"> — {{ subject.name }}</span>
                                    </span>
                                    <span v-if="subject.program" class="rounded-full bg-teal-50 px-1.5 py-0.5 text-[10px] font-semibold text-teal-700 dark:bg-teal-950/30 dark:text-teal-300">
                                        {{ subject.program.code }}
                                    </span>
                                </label>
                                <p v-if="filteredSubjects.length === 0" class="py-2 text-center text-xs text-muted-foreground">No subjects found.</p>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <Input id="f-name" v-model="form.name" type="text" required placeholder="Auto-filled from subject" class="mt-1.5" :readonly="!editingClass" />
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <Input id="f-section" v-model="form.section" type="text" maxlength="5" placeholder="e.g. A" class="mt-1.5" />
                            <InputError class="mt-2" :message="form.errors.section" />
                        </div>
                        <div>
                            <Label for="f-school-year">School Year</Label>
                            <Input id="f-school-year" v-model="form.school_year" type="text" maxlength="9" placeholder="2025-2026" class="mt-1.5" />
                    ...
                    <div>
                        ...
                        <Input id="f-class-code" v-model="form.class_code" type="text" maxlength="30" placeholder="e.g. BSIT3A-S1" class="mt-1.5" />
                        <InputError class="mt-2" :message="form.errors.class_code" />
                    </div>

                    <div>
                        <Label for="f-description">Description</Label>
                        <textarea id="f-description" v-model="form.description" rows="3" class="mt-1.5 w-full resize-none rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-ring focus:ring-2 focus:ring-ring/30" />
                        <InputError class="mt-2" :message="form.errors.description" />
                    </div>

                    <label v-if="editingClass" class="flex cursor-pointer items-center gap-2.5 rounded-xl border border-border px-3 py-2 text-sm text-foreground">
                        <input v-model="form.is_active" type="checkbox" class="rounded accent-orange-500" />
                    ...
                    <div class="flex gap-2 pt-1">
                        ...
                        <Button type="submit" :disabled="form.processing" class="flex-1 border-0 bg-orange-500 text-white hover:bg-orange-600">
                            {{ form.processing ? 'Saving…' : editingClass ? 'Update' : 'Create' }}
                        </Button>
                    </div>
                </form>
            </div>
