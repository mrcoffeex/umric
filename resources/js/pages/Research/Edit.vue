...
interface Props {
    paper: {
        ...
        files?: Array<{ id: number; file_name: string; file_size: number; file_path: string }> | null;
    };
...
function addProponentSlot() {
    if (form.proponents.length >= 3) return;
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    >
                        ...
                        <Label class="mb-1.5 block text-sm font-medium text-foreground">Sustainable Development Goals</Label>
                        <MultiSelect
                    ...
                    >
                        ...
                        <Label class="mb-1.5 block text-sm font-medium text-foreground">Research Agenda</Label>
                        <MultiSelect
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center gap-2">
                            <div class="flex h-10 flex-1 items-center gap-2 rounded-lg border border-input bg-muted px-3 py-2 text-sm text-foreground">
                                <span class="rounded bg-orange-100 px-1.5 py-0.5 text-xs font-medium text-orange-700 dark:bg-orange-500/20 dark:text-orange-400">You</span>
                        ...
                        >
                            ...
                            <template v-else>
                                <div class="relative flex-1">
                                    ...
                                    />
                                    <ul v-if="searchResults.length" class="absolute z-20 mt-1 w-full overflow-hidden rounded-lg border border-border bg-card shadow-lg">
                                        <li
                                    ...
                                    <div v-else-if="searchQuery.trim().length >= 2" class="absolute z-20 mt-1 w-full rounded-lg border border-border bg-card px-3 py-2 text-xs text-muted-foreground shadow-lg">
                                        No students found.
                                    </div>
                                </div>
                            </template>
                    ...
                    >
                        ...
                        >
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="truncate text-sm font-medium text-foreground">{{ existingFile.file_name }}</p>
                                <p class="text-xs text-muted-foreground">{{ formatFileSize(existingFile.file_size) }}</p>
                            </div>
                            <span class="text-xs text-muted-foreground">Current file</span>
                        </div>
                        <!-- Replace file -->
                        <p class="mb-2 text-xs text-muted-foreground">{{ existingFile ? 'Upload a new file to replace the current one (optional).' : 'Upload a research file.' }}</p>
                        <FilePreview
                            :file="form.file"
