...
import { Check, Copy, GraduationCap, Link2, Pencil, Plus, Search, Trash2, Users } from 'lucide-vue-next';
...
function joinUrl(schoolClass: SchoolClass): string {
    return schoolClass.join_code ? `${window.location.origin}/classes/join/${schoolClass.join_code}` : '';
}
...
const totalStudents = computed(() => props.classes.reduce((sum, c) => sum + (c.members_count || 0), 0));
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-0.5 text-sm text-muted-foreground">Manage class sections and year levels by program.</p>
            </div>
            <Button @click="openNew" class="flex items-center gap-2 border-0 bg-orange-500 text-white shadow shadow-orange-500/20 hover:bg-orange-600">
        ...
        <div class="grid gap-3 sm:grid-cols-3">
            ...
            <div class="flex items-center gap-3 rounded-xl border border-border bg-card p-4">
                ...
                <div>
                    ...
                    <p class="text-lg font-bold text-foreground">{{ totalStudents }}</p>
                </div>
            </div>
        </div>
        ...
        >
            <div class="space-y-4">
                ...
                <!-- Empty State -->
                <div v-if="classes.length === 0" class="rounded-2xl border border-border bg-card p-12 text-center">
                    <div class="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-orange-50 dark:bg-orange-950/30">
                ...
                <div v-else class="space-y-3">
                    ...
                    >
                        ...
                        <div class="flex items-start gap-3 p-5">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    <a :href="classesShow.url({ class: schoolClass.id })" class="truncate font-bold text-foreground hover:text-orange-500">{{ schoolClass.name }}</a>
                                    <span class="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">Sec {{ schoolClass.section }}</span>
                                    <span v-if="schoolClass.school_year" class="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">{{ schoolClass.school_year }}</span>
                                    <span v-if="schoolClass.semester" class="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-semibold text-blue-700 dark:bg-blue-950/30 dark:text-blue-400">{{ schoolClass.semester === 1 ? '1st Sem' : '2nd Sem' }}</span>
                                    <span v-if="schoolClass.term" class="rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-semibold text-indigo-700 dark:bg-indigo-950/30 dark:text-indigo-400">{{ schoolClass.term }}</span>
                                    <span v-for="subject in schoolClass.subjects?.slice(0, 3)" :key="subject.id" class="rounded-full bg-violet-50 px-2 py-0.5 text-[10px] font-semibold text-violet-700 dark:bg-violet-950/30 dark:text-violet-400">{{ subject.code }}</span>
                                    <span :class="['rounded-full px-2 py-0.5 text-[10px] font-semibold', schoolClass.is_active ? 'bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-400' : 'bg-red-50 text-red-700 dark:bg-red-950/30 dark:text-red-400']">
                                        {{ schoolClass.is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </div>
                                <p class="mt-1 truncate text-xs text-muted-foreground">{{ schoolClass.description || 'No description' }}</p>
                                <p v-if="schoolClass.faculty?.name" class="mt-0.5 truncate text-xs text-muted-foreground">
                                    <span class="font-medium text-foreground">Faculty:</span> {{ schoolClass.faculty.name }}
                                </p>
                            </div>
                            <div class="flex shrink-0 items-center gap-1">
                                <Button variant="ghost" size="icon" class="h-8 w-8 text-muted-foreground hover:text-orange-500" @click="openEdit(schoolClass)">
                                    <Pencil class="h-3.5 w-3.5" />
                                </Button>
                                <Button variant="ghost" size="icon" class="h-8 w-8 text-muted-foreground hover:text-red-500" @click="deleteClass(schoolClass)">
                        ...
                        <div class="flex flex-wrap items-center gap-4 border-t border-border bg-muted/30 px-5 py-3 text-xs">
                            <span class="inline-flex items-center gap-1.5 font-medium text-muted-foreground">
                                <Users class="h-3.5 w-3.5" />
                                {{ schoolClass.members_count }} {{ schoolClass.members_count === 1 ? 'student' : 'students' }}
                            </span>
                            <span v-if="schoolClass.join_code" class="inline-flex items-center gap-1.5">
                                <Link2 class="h-3.5 w-3.5 text-green-500" />
                                <span class="font-mono font-semibold text-foreground">{{ schoolClass.join_code }}</span>
                                <button
                                    type="button"
                                ...
                                >
                                    <Check v-if="copiedId === schoolClass.id" class="h-3 w-3 text-green-500" />
                                    <Copy v-else class="h-3 w-3" />
                                    {{ copiedId === schoolClass.id ? 'Copied!' : 'Copy link' }}
                                </button>
                            </span>
            ...
            <div v-if="showForm" class="rounded-2xl border border-border bg-card p-5">
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <label class="flex cursor-pointer items-center gap-2.5">
                        ...
                        <span class="text-sm font-medium text-foreground">Is Active</span>
                        >
                    </label>
