...
const typeCounts = computed(() => {
    const counts: Record<string, number> = { info: 0, success: 0, warning: 0, danger: 0 };
...
const activeItemCount = computed(() => props.announcements.filter((a) => a.is_active).length);
...
<template>
    <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-foreground">Announcements</h1>
                <p class="mt-0.5 text-sm text-muted-foreground">Publish, pin, and target announcements by role.</p>
            </div>
            <button type="button" class="inline-flex items-center gap-2 rounded-lg bg-orange-500 px-3 py-2 text-sm font-semibold text-white hover:bg-orange-600" @click="openCreate">
        ...
        <div class="grid gap-3 sm:grid-cols-3">
            ...
            <div class="flex items-center gap-3 rounded-xl border border-border bg-card p-4">
                ...
                <div>
                    ...
                    <p class="text-lg font-bold text-foreground">{{ activeItemCount }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3 rounded-xl border border-border bg-card p-4">
                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-amber-50 dark:bg-amber-950/30">
                    <Pin class="h-5 w-5 text-amber-500" />
                </div>
                <div>
                    <p class="text-xs font-medium text-muted-foreground">Pinned</p>
                    <p class="text-lg font-bold text-foreground">{{ pinnedCount }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="flex flex-wrap gap-2 border-t border-border px-4 py-3">
                <button
                    v-for="tab in [
                        ...
                        { key: 'danger', label: 'Danger', count: typeCounts.danger },
                    ]"
                    :key="tab.key"
                    type="button"
                    :class="[
                ...
                >
                    {{ tab.label }}
                    <span class="rounded-full bg-black/5 px-1.5 py-0.5 text-[10px] dark:bg-white/10">{{ tab.count }}</span>
                </button>
            </div>
        </div>

        <!-- Empty State -->
        <div v-if="filteredAnnouncements.length === 0" class="rounded-2xl border border-border bg-card p-12 text-center">
            ...
            <p class="font-semibold text-foreground">{{ announcements.length === 0 ? 'No announcements yet' : 'No results' }}</p>
            <p class="mt-1 text-sm text-muted-foreground">{{ announcements.length === 0 ? 'Create your first announcement for users.' : 'Adjust your search or type filter.' }}</p>
        </div>
        ...
        <div v-else class="grid gap-4 lg:grid-cols-2">
            <article
                ...
                :key="item.id"
                :class="['overflow-hidden rounded-2xl border border-border bg-card border-l-4', typeBorderColors[item.type]]"
            >
                <div class="p-5">
                    <div class="mb-2 flex items-center justify-between gap-2">
                        <p class="line-clamp-1 text-base font-bold text-foreground">{{ item.title }}</p>
                        <div class="flex shrink-0 items-center gap-1.5">
                            <span :class="['rounded-full px-2 py-0.5 text-[11px] font-semibold', typeColors[item.type]]">{{ item.type }}</span>
                ...
                <div class="flex items-center justify-between gap-2 border-t border-border bg-muted/30 px-5 py-3 text-xs text-muted-foreground">
                    <div class="flex items-center gap-3">
                        <span>By {{ item.created_by_name }}</span>
                        <span class="text-border">·</span>
                        <span>{{ formatDate(item.published_at) }} → {{ formatDate(item.expires_at) }}</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <button
                            type="button"
                            @click="toggleActive(item)"
                            :class="['rounded-lg px-2.5 py-1 font-semibold', item.is_active ? 'bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-300' : 'bg-muted text-muted-foreground']"
                        >
