...
function onPaste(e: ClipboardEvent) {
    ...
    for (const part of parts) {
        if (tags.value.length >= props.maxTags) break
        if (part.length > 40) continue
        if (!tags.value.map(t => t.toLowerCase()).includes(part.toLowerCase())) {
...
<template>
  ...
  >
    ...
    />
    <span v-else class="text-xs text-muted-foreground">Max {{ props.maxTags }} tags</span>
  </div>
</template>
