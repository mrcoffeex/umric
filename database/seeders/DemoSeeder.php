...
/**
 * Large demo dataset for presentations and local walkthroughs.
 *
 * Run (after migrations):
 *   php artisan db:seed --class=DemoSeeder
 *
 * Optional env:
 *   DEMO_STUDENT_COUNT=1000
 *   DEMO_FACULTY_COUNT=10
 *   DEMO_CLASSES_PER_FACULTY=3
 */
class DemoSeeder extends Seeder
{
    private const DEMO_ADMIN_EMAIL = 'admin@demo.umric.test';

    private const DEMO_STAFF_EMAIL = 'staff@demo.umric.test';
    ...
    public function run(): void
    {
        ...
        if (User::query()->where('email', 'student0001@demo.umric.test')->exists()) {
            $this->command?->warn('Demo sample data already exists (student0001@demo.umric.test). Skipping.');
        ...
        $admin = $this->createUserWithProfile('Demo Administrator', self::DEMO_ADMIN_EMAIL, 'admin');
        $this->createUserWithProfile('Demo Research Staff', self::DEMO_STAFF_EMAIL, 'staff');
        ...
        if ($facultyContexts->isEmpty()) {
            $this->command?->error('No programs/subjects available for demo classes. Seed departments first.');
        ...
        $this->command?->info("Creating research papers for {$studentContexts->count()} students...");
        ...
        $this->command?->info(sprintf(
            'Demo seed complete: %d faculty, %d classes, %d students/papers.',
            $this->facultyCount,
            $classContexts->count(),
            $studentContexts->count(),
        ));
    }
    ...
    private function createFacultyContexts(): Collection
    {
        ...
        return collect(range(1, $this->facultyCount))->map(function (int $index) use ($programs) {
            /** @var Program $program */
            $program = $programs->get(($index - 1) % $programs->count());
            $faculty = $this->createUserWithProfile(
                sprintf('Demo Faculty %02d', $index),
                sprintf('faculty%02d@demo.umric.test', $index),
                'faculty',
                $program->department,
                $program,
            );
    ...
    private function createFacultyClasses(Collection $facultyContexts): Collection
    {
        return $facultyContexts->flatMap(function (array $facultyContext, int $facultyIndex) {
            ...
            $selectedSubjects = collect(range(1, $this->classesPerFaculty))
                ->map(fn (int $yearLevel) => $subjects->firstWhere('year_level', $yearLevel))
                ->filter();

            if ($selectedSubjects->count() < $this->classesPerFaculty) {
                $selectedSubjects = $selectedSubjects
                    ->concat($subjects->reject(fn (Subject $subject) => $selectedSubjects->contains('id', $subject->id)))
                    ->take($this->classesPerFaculty)
                    ->values();
            }

            return collect(range(1, $this->classesPerFaculty))->map(function (int $slot) use ($faculty, $program, $department, $selectedSubjects, $facultyIndex) {
                ...
                $class = SchoolClass::factory()
                    ...
                    ->create([
                        'description' => sprintf(
                            'Demo class %d for %s under %s',
                            ($facultyIndex * $this->classesPerFaculty) + $slot,
                            $program->name,
                            $faculty->name,
                        ),
                        'join_code' => null,
    ...
    {
        return collect(range(1, $this->studentCount))->map(function (int $index) use ($classContexts) {
            ...
            $student = $this->createUserWithProfile(
                sprintf('Demo Student %04d', $index),
                sprintf('student%04d@demo.umric.test', $index),
                'student',
                $department,
                $program,
            );
    ...
    /**
     * @return array{adviser: User, statistician: User}
     */
